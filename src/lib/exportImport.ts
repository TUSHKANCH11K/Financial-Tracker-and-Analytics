...
const INVALID_IMPORT_ERROR = 'Невалидный файл импорта. Проверьте формат данных и попробуйте ещё раз.'
...
const normalizeDate = (value: unknown): string => {
  const dateText = typeof value === 'string' ? value.trim() : String(value ?? '').trim()
...
const normalizeType = (value: unknown, index: number): Transaction['type'] => {
  const typeText = typeof value === 'string' ? value.trim().toLowerCase() : String(value ?? '').trim().toLowerCase()
  ...
  throw new Error(`${INVALID_IMPORT_ERROR} Поле "type" в записи №${index + 1} должно быть "income"/"expense" или "Доход"/"Расход".`)
}
...
const normalizeTransactionRecord = (value: unknown, index: number): Transaction => {
  if (!value || typeof value !== 'object') {
    throw new Error(`${INVALID_IMPORT_ERROR} Запись №${index + 1} не распознана.`)
  }
  ...
  const id = typeof record.id === 'string' && record.id.trim() ? record.id.trim() : `imported-${index + 1}`
  const category = typeof record.category === 'string' ? record.category.trim() : ''
  const comment = typeof record.comment === 'string' && record.comment.trim() ? record.comment.trim() : undefined

  if (!category) {
    throw new Error(`${INVALID_IMPORT_ERROR} Поле "category" в записи №${index + 1} обязательно.`)
  }
...
const normalizeExcelRecord = (row: Record<string, unknown>, index: number): Transaction => {
  const type = normalizeType(getRowValue(row, ['type', 'тип']), index)
  const amount = normalizeAmount(getRowValue(row, ['amount', 'сумма']))
  const category = typeof getRowValue(row, ['category', 'категория']) === 'string'
  ...
  const comment = typeof commentValue === 'string' && commentValue.trim() ? commentValue.trim() : undefined
...
export async function parseJsonTransactions(raw: string): Promise<Transaction[]> {
  if (!raw || !raw.trim()) {
    throw new Error(INVALID_IMPORT_ERROR)
  }
  ...
  try {
    ...
    if (error instanceof Error && error.message.includes('Невалидный файл импорта')) {
      throw error
    }
