...
describe("exportImport helpers", () => {
  ...
  it("parses a valid JSON array of transactions", async () => {
    await expect(
      parseJsonTransactions(JSON.stringify(validTransactions)),
    ).resolves.toEqual(validTransactions);
  });

  it("rejects invalid JSON structure with a clear error", async () => {
    await expect(
      parseJsonTransactions(JSON.stringify({ ok: true })),
    ).rejects.toThrow("Невалидный файл импорта");

    await expect(parseJsonTransactions("[]")).rejects.toThrow(
  ...
  it("rejects invalid Excel rows with a clear error", async () => {
    ...
    await expect(parseExcelTransactions(file)).rejects.toThrow(
      "Невалидный файл импорта",
    );
