...
import { aggregateByCategory, aggregateByDay, getBalanceSummary } from './aggregations'
...
describe('aggregations', () => {
  ...
  it('getBalanceSummary корректно считает доходы, расходы и баланс за месяц', () => {
    expect(getBalanceSummary(transactions, '2026-08')).toEqual({
      income: 2000,
      expense: 450,
      balance: 1550,
    })
  ...
  it('getBalanceSummary возвращает нулевые значения, если за месяц нет операций', () => {
    expect(getBalanceSummary(transactions, '2026-09')).toEqual({
      income: 0,
      expense: 0,
      balance: 0,
    })
